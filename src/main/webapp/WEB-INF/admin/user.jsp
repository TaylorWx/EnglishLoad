...
<jsp:include page="adminheader.jsp"></jsp:include>
        ...
        <c:forEach items="${userList}"  var="user">
     	<tr>
  			...
  			<td>${user.email}</td>
  			<td>${user.roleId}</td>
  			<td><a href = "JavaScript:void(0)" onclick = "document.getElementById('light${user.id}').style.display='block';document.getElementById('fade').style.display='block'"">修改</a></td>
  			<td>   <a href="${baseurl}/delete?id=${user.id}">删除</a> </td>
  			<td>
  			
  		<div class="white_content" id="light${user.id}">更新用户信息：
  	...
  	<table>
  		<tr> 
  			<th class="lable">用户名</th>
  			<td><input type="text" name="username" value="${user.username}" /></td>
  		</tr>
  		<tr>
  			 
  			<th class="lable">密码</th>
  			<td><input type="password" name="password" value="${user.password}" /></td>
  		</tr>
  		<tr>
  			 
  			<th class="lable">电话</th>
  			<td><input type="text" name="phone" value="${user.phone}" /></td>
  		</tr>
  		<tr>
  			 
  			<th class="lable">邮箱</th>
  			<td><input type="text" name="email" value="${user.email}" /></td>
  		</tr>
  		<tr>
  			 
  			<th class="lable">加密盐</th>
  			<td><input type="text" name="salt" value="${user.salt}" /></td>
  		</tr>
  		<tr>
  			 
  			<th class="lable">角色</th>
  			<td><input type="text" name="roleId"  value="${user.roleId}" /></td>
  		</tr>
  		<tr>
  			<td colspan="2"><input type="submit" class="button" value="修改"> </td>
  		</tr>
  	</table>
  	</form> 
